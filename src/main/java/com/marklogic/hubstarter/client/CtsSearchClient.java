...
public class CtsSearchClient {
    ...
    public CtsSearchClient(String host, int port, String user, String password, boolean dhs, String searchOptions) {
        this.host = host;
        this.port = port;
        this.user = user;
    ...
    public void search(String searchString) throws Exception {
        ...
        try {
            ...
            String rawXml = "<cts:word-query xmlns:cts='http://marklogic.com/cts'><cts:text xml:lang='en'>total</cts:text></cts:word-query>";

            //String rawJson = "{\"wordQuery\":{\"text\":[\"blade\"], \"options\":[\"lang=en\"], \"weight\":12}}";

            //StructuredQueryBuilder qb = new StructuredQueryBuilder(searchOptions);
            //RawCtsQueryDefinition querydef = queryMgr.newRawCtsQueryDefinition(
            //    new StringHandle(rawXml).withFormat(Format.XML)
            //);

            RawCtsQueryDefinition querydef = queryMgr.newRawCtsQueryDefinition(
                new StringHandle(rawXml).withFormat(Format.JSON)
            );
    ...
    protected void printResults(SearchHandle resultsHandle, QueryDefinition querydef) {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        CtsSearchClient searchClient = new CtsSearchClient(props.getProperty("host", "localhost"),
